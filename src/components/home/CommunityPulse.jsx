...
function StatCell({ label, value, icon, subtitle }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-4" style={{ background: 'var(--surface-strong)' }}>
      <div
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl"
        style={{ background: 'var(--purple-ghost)' }}
      >
        {icon === 'thread' && (
          <svg className="h-4 w-4" style={{ color: iconColor }} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-xs leading-tight text-text-subtle">{label}</p>
        {subtitle && <p className="text-[10px] leading-tight text-text-subtle">{subtitle}</p>}
      </div>
    </div>
  );
}
